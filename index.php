...
if (isset($sort_j)) // Get sorting column
 {
  if ($sort_j === "1")
   {
...
if (isset($sort_s)) // Get sorting by status
 {
  if ($sort_s === "1")
   {
...
if (isset($_GET['search'])) // If a search string is present, handle that. Handled via GET for bookmarking a search.
 {
...
else // If no search term present, just get all case id's for device counting.
 {
  ...
  foreach ($row_active as $entry) // Count and update devicecount in case they loses track.
   {
